...
const PosContainer = () => {
  ...
  const [isMultipleProductAdded, setIsMultipleProductAdded] = useState(false);
  ...
  const emitOnAddMultiplePosItemHandler = (itemObjArr) => {
    if (!itemObjArr || itemObjArr.length === 0) {
      toast.dismiss();
      toast.error("Oops! Something went wrong!");
      return;
    }
    ...
    itemObjArr.forEach((itemObj) => {
      const productIndex = productList.findIndex(
        (item) => item.id === parseInt(itemObj?.id)
      );
      if (productIndex !== -1) {
        const selectedProduct = productList[productIndex];
        // Initialize bill if it's empty
    ...
    setPosBill(updatedPosBill);
    billSaveToLocalStorage(updatedPosBill);
    setIsMultipleProductAdded(true);
    toast.dismiss();
